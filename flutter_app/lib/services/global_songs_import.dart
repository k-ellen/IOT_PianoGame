...
Future<void> importGlobalSongsToFirestore() async {
  ...
  final rootRef = storage.ref(
    'Global Songs',
  ); //Gives a Reference that points to a folder in Storage
  ...
  Future<void> walkFolder(Reference folderRef) async {
    ...
    for (final fileRef in listResult.items) {
      ...
      final parts = fullPath.split(
        '/',
      ); //Parses the path into a list, for exemple parts = ["Global Songs", "Classical", "Mozart", "song1.mid"]
      ...
      final artistName =
          parts[parts.length - 2]; //The folder that is one before the end
  ...
  await walkFolder(
    rootRef,
  ); //Responsible for first running the walkFolder function on rootRef = Global Songs
